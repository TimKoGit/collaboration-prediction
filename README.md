...
Данные автоматически загружаются при первом запуске любой команды (тренировка, оценка или экспорт). Проект использует DVC для управления датасетами. **DVC репозиторий и отслеживание данных настраиваются автоматически при первом запуске.**

- Датасет **ogbl-collab** будет автоматически скачан в `./data/ogbl_collab/` при первом запуске
- Данные версионируются через DVC
- Если данные уже есть локально, они будут использованы без повторной загрузки
