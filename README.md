...
2.1. **Установите зависимости для экспорта моделей (опционально):**
...
2.2. **Установите зависимости для inference сервера (опционально):**
...
2.3. **Установите зависимости для разработки (опционально):**
...
3. **Установите pre-commit хуки (рекомендуется для разработки):**
...
3.1 **Для самостоятельного запуска проверок используйте следующую команду**
...
Проект использует внешнюю папку для хранения данных и DVC репозитория.
...
Мотивация этой проверки состоит в том, что при переносе в ONNX формат пришлось убрать работу с разреженными матрицами, из-за чего архитектуру была немного изменена.
...
- ✅ **Успех**: Если выходы совпадают (по умолчанию: относительная погрешность `rtol=1e-2`, абсолютная `atol=5e-2`), вы увидите сообщение `✓ ONNX model verification passed!` с информацией о максимальной и средней разнице между выходами.
